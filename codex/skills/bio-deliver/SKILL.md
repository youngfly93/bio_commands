...
name: bio-deliver
description: >-
  一键交付打包：从最终质量审计到生成 Windows 兼容 ZIP 全流程自动化。
  触发条件：用户说"打包交付"、"做交付包"、"deliver"、"出 ZIP"、"导师/客户要交付了"、
  或项目完结需要按标准目录结构组装最终交付物时使用。
  不适用于：还在分析阶段、还有 P0/P1 问题待修（先用 $bio-audit-fix）、仅需中间汇总。
---
...
调用 $bio-result-auditor 的审计逻辑对照 plan.md 做最终审计：
- 如果存在 P0/P1 问题，**中止打包**并输出问题清单
- 仅 P2/P3 问题时提醒但继续

## 步骤 2：收集交付物

在项目根目录创建 `delivery/`，按中文编号组织：
...
对 `delivery/` 中所有文件执行 AI 痕迹扫描（等效 $bio-ai-clean）：
