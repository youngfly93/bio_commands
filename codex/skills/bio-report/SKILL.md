...
name: bio-report
description: >-
  生成中文生信分析 Word 交付报告（.docx），含格式规范、图表嵌入、XML 校验。
  触发条件：用户说"生成报告"、"写 Word 报告"、"导出 docx"、"出交付报告"、"按 plan 写报告"，
  或在分析完成后需要打包成 Word 文档交付时使用。
  不适用于：报告内容尚未确定、还没做完分析、仅需 markdown 输出的场景。
---
...
2. 扫描 `results/`、`figures/` 收集所有可用的图表和结果
...
3. **Markdown 残留**：扫描文档文本是否有 `**`, `##`, `` ` ``, `- [ ]` 等 markdown 标记残留，发现则清除

## 注意事项

- 优先使用 python-docx / docx 操作模板生成 `.docx`
- 图表编号连续，与正文引用一致
- 表格使用三线表风格
- 避免 AI 套话（参考 $bio-ai-clean 中的关键词列表）
