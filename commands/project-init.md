...
> 填写时把所有 `?` 替换为具体值；针对不同分析类型补充对应步骤（DEG 阈值、聚类分辨率、OG 鉴定参数等）。

### 3.2 `CLAUDE.md`（Agent 行为配置）
...
### 3.3 `execution_log.md`（执行/审计日志模板）
...
### 3.4 `numeric_reference.tsv` 与 `report_claims.tsv`（数值追踪）

```tsv
key	value	source_file	note
```

两张表用于报告写作时校对数值：左表是源数据真值，右表是报告里出现的数值声明，最终需保证一一对应。

## 步骤 4：引导填写

展示生成的 plan.md，标注需要填写的 `?` 占位符：
- 固定参数（DEG 阈值、分组定义等）
- 项目背景（数据来源、实验设计）
- QC 断言中的具体数值（如 sample_total）

帮助用户逐项填写，或根据已有数据文件自动推断。
